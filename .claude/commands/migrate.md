...
- The `firmware/` directory is the canonical source - changes here are rsynced to $QMK_HOME via `mise run sync`
- Use `mise run reset <keyboard>` to remove and re-sync (clears legacy files that rsync doesn't remove)
